...
enum tfa_error tfa_load_cnt(void *cnt, int length) 
{
	...
	if (length > TFA_MAX_CNT_LENGTH) {
		pr_err("tfa98xx: incorrect length\n");
		return tfa_error_container;
	}

	if (HDR(cntbuf->id[0],cntbuf->id[1]) == 0) {
		pr_err("tfa98xx: header is 0\n");
		return tfa_error_container;
	}

	if ( (HDR(cntbuf->id[0],cntbuf->id[1])) != paramsHdr ) {
		pr_err("tfa98xx: wrong header type: 0x%02x 0x%02x\n", cntbuf->id[0],cntbuf->id[1]);
		return tfa_error_container;
	}

	if (cntbuf->size == 0) {
		pr_err("tfa98xx: data size is 0\n");
		return tfa_error_container;
	}

	/* check CRC */
	if ( tfaContCrcCheckContainer(cntbuf)) {
		pr_err("tfa98xx: CRC error\n");
		return tfa_error_container;
	}
	...
	if ((cntbuf->subversion[1] != NXPTFA_PM_SUBVERSION) &&
		...
		pr_err("tfa98xx: container sub-version not supported: %c%c\n",
...
nxpTfaFileDsc_t *tfacont_getfiledata(struct tfa_device *tfa, int prof_idx, enum nxpTfaHeaderType type)
{
	...
	if (tfa->cnt == NULL) {
		pr_err("tfa98xx: invalid pointer to container file\n");
		return NULL;
	}

	dev = tfaContGetDevList(tfa->cnt, tfa->dev_idx);
	if (dev == NULL) {
		pr_err("tfa98xx: invalid pointer to container file device list\n");
		return NULL;
	}
	...
	if (prof == NULL) {
		pr_err("tfa98xx: invalid pointer to container file profile list\n");
		return NULL;
	}
...
static enum Tfa98xx_Error tfaContWriteVstep(struct tfa_device *tfa,  nxpTfaVolumeStep2File_t *vp, int vstep)
{
	...
	if (vstep < vp->vsteps) {
		...
		pr_err("tfa98xx: Incorrect volume given. The value vstep[%d] >= %d\n", vstep , vp->vsteps);
		err = Tfa98xx_Error_Bad_Parameter;
	}

	if (tfa->verbose) pr_err("tfa98xx: vstep[%d][%d]\n", tfa->dev_idx, vstep);
						    ...
						    struct nxpTfaVolumeStepMessageInfo *old_msg, int enable_partial_update)
{
	...
	if (enable_partial_update) {
		if (new_msg->MessageType != old_msg->MessageType) {
			pr_err("tfa98xx: Message type differ - Disable Partial Update\n");
			enable_partial_update = 0;
		} else if (tfaContGetmsgLen(new_msg) != tfaContGetmsgLen(old_msg)) {
			pr_err("tfa98xx: Message Length differ - Disable Partial Update\n");
			enable_partial_update = 0;
		}
	}
	...
	if (tfa_needs_reset(tfa) == 1) {
		if (new_msg->MessageType == 0) {
			...
			if (tfa->verbose)
				pr_err("tfa98xx: P-ID for SetAlgoParams modified!\n");
		} else if (new_msg->MessageType == 2) {
			cmdid[2] = SB_PARAM_SET_MBDRC;
			if (tfa->verbose)
				pr_err("tfa98xx: P-ID for SetMBDrc modified!\n");
		}
	}
	...
	if (enable_partial_update) {
		...
		if (!partial)
			pr_err("tfa98xx: Partial update memory error - Disabling\n");
	}
	...
	if (partial) {
		...
		*p++ = 0x00;
		...
		if (trim == partial) {
			pr_err("tfa98xx: No Change in message - discarding %d bytes\n", len);
			len = 0;
			
		} else if (trim < (partial + len - 3)) {
			pr_err("tfa98xx: Using partial update: %d -> %d bytes\n", len , (int)(trim-partial+3));
			...
			pr_err("tfa98xx: Partial too big - use regular update\n");
		}
	}
	...
	if (use_partial_coeff) {
		...
		if (tfa->verbose)
			pr_err("tfa98xx: Command-ID used: 0x%02x%02x%02x \n", cmdid[0], cmdid[1], cmdid[2]);
...
static enum Tfa98xx_Error tfaContWriteVstepMax2(struct tfa_device *tfa, nxpTfaVolumeStepMax2File_t *vp, int vstep_idx, int vstep_msg_idx)
{
	...
	if (vstep_idx >= vp->NrOfVsteps) {
		pr_err("tfa98xx: Volumestep %d is not available \n", vstep_idx);
		return Tfa98xx_Error_Bad_Parameter;
	}

	if (tfa->p_regInfo == NULL) {
		if (tfa->verbose)
			pr_err("tfa98xx: Inital vstep write\n");
		enp = 0;
 	}
	...
	if (enp) {
		...
		if (nrMessages != p_msgInfo->NrOfMessages) {
			pr_err("tfa98xx: Message different - Disable partial update\n");
			enp = 0;
		}
	}
...
enum Tfa98xx_Error tfaContWriteFile(struct tfa_device *tfa,  nxpTfaFileDsc_t *file, int vstep_idx, int vstep_msg_idx)
{
	...
	case infoHdr:
		...
		break;
	default:
		pr_err("tfa98xx: Header is of unknown type: 0x%x\n", type);
		return Tfa98xx_Error_Bad_Parameter;
	}
...
#ifdef TFA_DEBUG
	if (tfa->verbose)
		pr_err("tfa98xx: bitfield: %s=0x%x (0x%x[%d..%d]=0x%x)\n", tfaContBfName(bfUni.field, tfa->rev), value,
...
static enum Tfa98xx_Error tfaRunWriteDspMem(struct tfa_device *tfa, nxpTfaDspMem_t *cfmem)
{
	...
	for(i=0;i<cfmem->size;i++) {
		if (tfa->verbose)
			pr_err("tfa98xx: dsp mem (%d): 0x%02x=0x%04x\n", cfmem->type, cfmem->address, cfmem->words[i]);
...
static enum Tfa98xx_Error tfaRunWriteFilter(struct tfa_device *tfa, nxpTfaContBiquad_t *bq) 
{
	...
	if (tfa->verbose) {
		if(channel == 2)
			pr_err("tfa98xx: filter[%d,S]", bq->aa.index);
		else if(channel == 1)
			pr_err("tfa98xx: filter[%d,P]", bq->aa.index);
		else
			pr_err("tfa98xx: filter[%d]", bq->aa.index);
	}
	...
	for(i=0; i<runs; i++) {
		...
		if(dmem == Tfa98xx_DMEM_ERR) {
			if (tfa->verbose) {
				pr_err("tfa98xx: Warning: XFilter settings are applied via msg file (ini filter[x] format is skipped).\n");
			}
...
static enum Tfa98xx_Error tfaRunWriteRegister(struct tfa_device *tfa, nxpTfaRegpatch_t *reg)
{
	...
	if (tfa->verbose)
		pr_err("tfa98xx: register: 0x%02x=0x%04x (msk=0x%04x)\n", reg->address, reg->value, reg->mask);
...
enum Tfa98xx_Error tfaContWriteFiles(struct tfa_device *tfa) 
{
	...
	for(i=0;i<dev->length;i++) {
		...
		if  ( dev->list[i].type == dscSetInputSelect ||
			...
			if (tfa->verbose) {
				pr_err("tfa98xx: command: %s=0x%02x%02x%02x \n",
	...
	for(i=0;i<prof->length;i++) {
		switch (prof->list[i].type) {
			...
			case dscSetVddpConfig:
					...
					if (tfa->verbose) {
						pr_err("command: %s=0x%02x%02x%02x \n",
...
enum Tfa98xx_Error tfa_write_filters(struct tfa_device *tfa, int prof_idx) 
{
	...
	if (tfa->verbose) {
		pr_err("tfa98xx: ----- profile: %s (%d) -----\n", tfaContGetString(tfa->cnt, &prof->name), prof_idx);
		pr_err("tfa98xx: Waiting for CLKS... \n");
	}
	...
	if(i==0) {
		if (tfa->verbose)
			pr_err("tfa98xx: Unable to write filters, CLKS=0 \n");
...
static enum Tfa98xx_Error get_sample_rate_info(struct tfa_device *tfa, nxpTfaProfileList_t *prof, nxpTfaProfileList_t *previous_prof, int fs_previous_profile)
{
	...
	*/
	...
	if(fs_next_profile != fs_default_profile) {
		if (tfa->verbose)
			pr_err("tfa98xx: Writing delay tables for AUDFS=%d \n", fs_next_profile);
...
enum Tfa98xx_Error tfaContWriteProfile(struct tfa_device *tfa, int prof_idx, int vstep_idx) 
{
	...
	if ( !prof || !previous_prof ) {
		pr_err("tfa98xx: Error trying to get the (previous) swprofile \n");
		return Tfa98xx_Error_Bad_Parameter;
	}

	if (tfa->verbose) {
		tfa98xx_trace_printk("tfa98xx: device:%s profile:%s vstep:%d\n", tfaContDeviceName(tfa->cnt, tfa->dev_idx),
	...
	if (prof->group == previous_prof->group && prof->group != 0) {
		if (tfa->verbose) {
			pr_err("tfa98xx: The new profile (%s) is in the same group as the current profile (%s) \n", 
		...
		if (((tfa->tfa_family == 2) && (TFA_GET_BF(tfa, MANSTATE) >= 6)) || (tfa->tfa_family != 2)) {
			...
			if (tries > 100) {
				pr_err("tfa98xx: Wait for PLL powerdown timed out!\n");
				return Tfa98xx_Error_StateTimedOut;
			}
		} else {
			pr_err("tfa98xx: No need to go to powerdown now \n");
		}
	}
	...
	if (tfa->verbose) {
		pr_err("tfa98xx: ---------- default settings profile: %s (%d) ---------- \n", 
	...
	if (tfa->verbose)
		pr_err("tfa98xx: ---------- new settings profile: %s (%d) ---------- \n", 
				tfaContGetString(tfa->cnt, &prof->name), prof_idx);
	...
	if(tfa->ext_dsp != 0) {
			...
			for(i=0;i<previous_prof->length;i++) {
				...
				if(i < previous_prof->length) {
					if(previous_prof->list[i].type == dscFile || previous_prof->list[i].type == dscPatch) {
						...
						if ( tfa->verbose && k==0) {
							pr_err("---------- files default profile: %s (%d) ---------- \n",
		...
		if ( tfa->verbose) {
			pr_err("tfa98xx: ---------- files new profile: %s (%d) ---------- \n",
					tfaContGetString(tfa->cnt, &prof->name), prof_idx);
		}
	}
			...
			case dscCmd:
				...
				if(tfa->ext_dsp != 0) {
					...
					if (tfa->verbose) {
						...
						pr_err("tfa98xx: Writing cmd=0x%02x%02x%02x \n", (uint8_t)cmd_id[0], (uint8_t)cmd_id[1], (uint8_t)cmd_id[2]);
					}
				}
				break;
			default:
...
int tfa_tib_dsp_msgmulti(struct tfa_device *tfa, int length, const char *buffer)
{
	...
	if (blob == NULL) {
		if (tfa->verbose)
				pr_err("%s, Creating the multi-message \n\n", __FUNCTION__);
