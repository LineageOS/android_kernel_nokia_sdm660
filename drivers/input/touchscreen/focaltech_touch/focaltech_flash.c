...
/************************************************************************
* Name: fts_ctpm_get_bin_ver
* Brief:  get app file version
* Input:
* Output:
* Return: fw version
***********************************************************************/
int fts_ctpm_get_bin_ver(void)
{
    int i_ret = 0;

    if (fts_updatefun_curr.get_app_bin_file_ver)
        i_ret = fts_updatefun_curr.get_app_bin_file_ver(fw_app_bin);
...
*         0 - vendor id invalid
***********************************************************************/
int fts_ctpm_check_fw_ver(struct i2c_client *client)
{
    u8 uc_tp_fm_ver;
    u8 uc_host_fm_ver = 0;
    int ret = 0;
    ...
    if ((uc_tp_fm_ver < uc_host_fm_ver) || (uc_tp_fm_ver > FTS_FW_VER_EXC_VAL)) //20170505 JanChen modify. //org: if ( (uc_tp_fm_ver < uc_host_fm_ver )
    {
        return 1;
    }
    else
...
/************************************************************************
* Name: fts_ctpm_check_fw_bin_ver
* Brief: Check vendor id is valid or not
* Input:
* Output:
* Return: 1 - vendor id valid
*         0 - vendor id invalid
***********************************************************************/
int fts_ctpm_check_fw_bin_ver(struct i2c_client *client)
{
    u8 uc_tp_fm_ver;
    u8 uc_host_fm_ver = 0;

    fts_i2c_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);
    uc_host_fm_ver = fts_ctpm_get_bin_ver();

    FTS_DEBUG("[UPGRADE]: uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x!!", uc_tp_fm_ver, uc_host_fm_ver);
    if (strstr(saved_command_line, "androidboot.device=PL2") != NULL)
    {
        if ((uc_tp_fm_ver != uc_host_fm_ver ) || (uc_tp_fm_ver > FTS_FW_VER_EXC_VAL))
        {
            return 1;
        }
        else
