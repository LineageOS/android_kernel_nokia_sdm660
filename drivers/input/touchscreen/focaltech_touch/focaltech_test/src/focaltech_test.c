...
static int fts_test_entry(char *ini_file_name)
{
    ...
    {
       FTS_TEST_INFO("@@ true == start_test_tp() pass!!!!!!!!!!!");
       FTS_TEST_DBG("TotalTestResult = %d ,LcdReTryTestResult = %d,cbReTryTestResult = %d\n",TotalTestResult,LcdReTryTestResult,cbReTryTestResult);
       if( (LcdReTryTestResult == false) || (TotalTestResult == false) || (cbReTryTestResult == false) 
       	|| (RawDataTotalTestResult == false) || (LCDTotalTestResult == false)){
    ...
    {
         FTS_TEST_INFO("@@ false == start_test_tp() fail!!!!!!!!!!!");
        TestResultLen += sprintf(TestResult+TestResultLen,"Tp test failure. \n\n");
        FTS_TEST_INFO("@@ tp test failure!!!!!!!!!!");
        ResultSelfTest = false;
	 TotalTestResult = true;
