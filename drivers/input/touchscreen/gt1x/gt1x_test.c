...
int parse_config(unsigned char *config)
{
	...
	pr_err("product id %s", gt1x_version.product_id);
	...
	pr_err("drv %d, sen %d", sys.driver_num, sys.sensor_num);
...
int check_version(unsigned char *ini_version)
{
	...
	pr_err("ic version %s and ini %s compare result %d", tmp, ini_version, ret);
...
int read_raw_data(unsigned short *data, int len)
{
	...
	if (ret < 0) {
		...
		pr_err("enter rawdiff mode fail.");
	...
	//pr_err("0x%X read %d*%d data", sys.reg_rawdata_base, sys.sensor_num, sys.driver_num);

	ret = gt1x_i2c_read(sys.reg_rawdata_base, buf, sys.sensor_num * sys.driver_num * 2);

	if (ret < 0) {

		pr_err("read raw data fail.");
		free(buf);
		return ret;
...
void write_test_params_bak(u8 * config)
{
	...
	pr_err("write test parameters sen%d*drv%d", sys.max_sensor_num, sys.max_driver_num);
}
...
static int _calculate_short_resistance(strShortRecord r_data, unsigned short short_r_th, unsigned char flag)
{
	...
	pr_err("calc:short_code=%d,r_th=%d,warn=%d", short_code, short_r_th, warn_threshold);

	if (short_code < (short_r_th * 10)) {

		//pr_err("uplaod short position %d,master 0x%X,slave 0x%X",r_data.position,r_data.master,r_data.slave);
		...
		//      return 0;
		//}
		//pr_err("uplaod short code %d,master 0x%X,slave 0x%X",short_code,r_data.master,r_data.slave);
...
int short_test_analysis(unsigned char *result, int t_size)
{
	...
	for (i = 0; i < sys.max_driver_num + sys.max_sensor_num; i++) {
		...
		// pr_err("self_data[%d]%d",i,self_data[i]);
	}

	i2c_read_data(0x8802, short_status, 3);

	pr_err("Tx&Tx:%d,Rx&Rx:%d,Tx&Rx:%d", short_status[0], short_status[1], short_status[2]);
...
s32 load_code_and_check(unsigned char *codes, int size)
{
	...
	if (count < 5) {
		...
		pr_err("Burn DSP code successfully!\n");
...
long ini_read_float(char *file, const char *key)
{
	...
	pr_err("ini read float ascii:%s", tmp);

	value = atof((const char *)tmp);

	pr_err("ini read float value:%ld", value);
...
static ssize_t gtp_sysfs_openshort_show(struct device *dev, struct device_attribute *attr, char *buff)
{
	...
	if (ret < 0) {
		...
		len += sprintf((char *)&buff[len], "test error code 0x%ld", ret);
	...
	pr_err("This is gtp_sysfs_openshort_show result len = %ld\n\n",len);
	return len;
}
...
static ssize_t gtp_sysfs_openshort_store(struct device *dev, struct device_attribute *attr, const char *buff, size_t count)
{
	...
	if (fp == NULL) {
		...
		pr_err("open /sdcard/test.csv fail");
...
s32 gtp_test_sysfs_init(void)
{
	...
	s32 ret;
	pr_err("GT1X debug sysfs enter  gtp_test_sysfs_init!\n");
	...
	if (ret) {
		...
		pr_err("%s: sysfs_create_openshort_file failed\n", __func__);

		return ret;

	}

	pr_err("GT1X debug sysfs create success!\n");
