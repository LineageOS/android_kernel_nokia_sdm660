...
const char CFG_SSL_8719 = '[';
const char CFG_SSR_8719 = ']';
const char CFG_NIS_8719 = ':';  /* Separator between name and index */
const char CFG_NTS_8719 = '#';  /* annotator */
const char CFG_EQS_8719 = '=';  /* The equal sign */
...
int fts_strncmp_8719(const char *cs, const char *ct, size_t count)
{
    u8 c1 = 0, c2 = 0;
...
Note:
*************************************************************/
int ini_get_key_8719(char *filedata, char *section, char *key, char *value)
{
    int i = 0;
    int ret = -2;
    for (i = 0; i < test_data.ini_keyword_num; i++) {
        if (fts_strncmp_8719(section, test_data.ini_data[i].section_name,
        ...
        if (strlen(key) == test_data.ini_data[i].key_name_len) {
            if (fts_strncmp_8719(key, test_data.ini_data[i].key_name,  test_data.ini_data[i].key_name_len) == 0)
...
Note:
*************************************************************/
char *ini_str_trim_r_8719(char *buf)
{
    int len, i;
...
Note:
*************************************************************/
char *ini_str_trim_l_8719(char *buf)
{
    int len, i;
    char tmp[MAX_CFG_BUF_LINE_LEN] = { 0 };
...
static long fts_atol(char *nptr)
{
    ...
    /* skip whitespace */
    while ( isspace_8719((int)(unsigned char)*nptr) )
        ++nptr;
...
int fts_atoi_8719(char *nptr)
{
    return (int)fts_atol(nptr);
}

int init_key_data_8719(void)
{
    int i = 0;
...
Note:
*************************************************************/
int ini_get_key_data_8719(char *filedata)
{
    ...
    while (1) { /*find section */
        ...
        dataoff += n;
        n = strlen(ini_str_trim_l_8719(ini_str_trim_r_8719(buf1)));
        if (n == 0 || buf1[0] == CFG_NTS_8719)
            continue;       /* A blank line or a comment line */
        ret = CFG_ERR_FILE_FORMAT;
        //get section name
        if (n > 2 && ((buf1[0] == CFG_SSL_8719 && buf1[n - 1] != CFG_SSR_8719))) {
            FTS_TEST_ERROR("Bad Section:%s\n",  buf1);
            goto cfg_scts_end;//bad section
        }

        if (buf1[0] == CFG_SSL_8719) {
...
int GetPrivateProfileString_8719(char *section, char *ItemName, char *defaultvalue, char *returnValue, char *IniFile)
{
    ...
    if (ini_get_key_8719(IniFile, section, ItemName, value) < 0) {
...
int init_interface(char *ini)
{
    ...
    /* IC type */
    GetPrivateProfileString_8719("Interface", "IC_Type", "FT5X36", str, ini);
    ic_type = fts_ic_table_get_ic_code_from_ic_name_8719(str);
    if (0 == ic_type) {
    ...
    /* Normalize Type */
    GetPrivateProfileString_8719("Interface", "Normalize_Type" , 0 , str, ini);
    test_data.screen_param.normalize = fts_atoi_8719(str);
...
***********************************************************************/
int set_param_data_8719(char *test_param)
{
    int ret = 0;

    FTS_TEST_FUNC_ENTER();

    test_data.ini_data = (struct _ini_data *)fts_malloc_8719(sizeof(struct _ini_data) * MAX_KEYWORD_NUM);
    if (NULL == test_data.ini_data) {
        FTS_ERROR("mallock memory for ini_data fail");
        goto SET_PARAM_ERR;
    }

    ret = ini_get_key_data_8719(test_param);
    if (ret < 0) {
        FTS_TEST_ERROR("ini_get_key_data_8719 error.");
        return ret;
    }
...
int fts_test_get_testparam_from_ini(char *config_name)
{
    ...
    ini_file_data = fts_malloc_8719(inisize + 1); // 1: end mark
    if (NULL == ini_file_data) {
        FTS_TEST_ERROR("fts_malloc_8719 failed in function:%s",  __func__);
        return -ENOMEM;
    }
    ...
    ret = set_param_data_8719(ini_file_data);
    if (ret) {
