...
struct fts_mode_flag g_fts_mode_flag_8719;
...
static ssize_t fts_touch_glove_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    fts_i2c_read_reg_8719(client, FTS_REG_GLOVE_MODE_EN, &val);
    count = snprintf(buf, PAGE_SIZE, "Glove Mode: %s\n", g_fts_mode_flag_8719.fts_glove_mode_flag ? "On" : "Off");
...
static ssize_t fts_touch_glove_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        if (!g_fts_mode_flag_8719.fts_glove_mode_flag) {
            FTS_INFO("[Mode]enter glove mode");
            ret = fts_enter_glove_mode(client, true);
            if (ret >= 0) {
                g_fts_mode_flag_8719.fts_glove_mode_flag = true;
            }
        }
        ...
        if (g_fts_mode_flag_8719.fts_glove_mode_flag) {
            ...
            if (ret >= 0) {
                g_fts_mode_flag_8719.fts_glove_mode_flag = false;
            }
        }
    }
    FTS_INFO("[Mode]glove mode status:  %d", g_fts_mode_flag_8719.fts_glove_mode_flag);
    return count;
}
...
int fts_enter_glove_mode( struct i2c_client *client, int mode)
{
    ...
    ret = fts_i2c_write_reg_8719( client, buf_addr[0], buf_value[0]);
    if (ret < 0) {
...
static ssize_t fts_touch_cover_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    mutex_lock(&input_dev->mutex);
    fts_i2c_read_reg_8719(client, FTS_REG_COVER_MODE_EN, &val);
    count = snprintf(buf, PAGE_SIZE, "Cover Mode: %s\n", g_fts_mode_flag_8719.fts_cover_mode_flag ? "On" : "Off");
...
static ssize_t fts_touch_cover_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    client = ts_data->client;
    if (FTS_SYSFS_ECHO_ON(buf)) {
        if (!g_fts_mode_flag_8719.fts_cover_mode_flag) {
            FTS_INFO("[Mode]enter cover mode");
            ret = fts_enter_cover_mode(client, true);
            if (ret >= 0) {
                g_fts_mode_flag_8719.fts_cover_mode_flag = true;
            }
        }
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        if (g_fts_mode_flag_8719.fts_cover_mode_flag) {
            ...
            if (ret >= 0) {
                g_fts_mode_flag_8719.fts_cover_mode_flag = false;
            }
        }
    }
    FTS_INFO("[Mode]cover mode status:  %d", g_fts_mode_flag_8719.fts_cover_mode_flag);
    return count;
}
...
static ssize_t fts_touch_charger_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    count = snprintf(buf, PAGE_SIZE, "Charge Mode: %s\n", g_fts_mode_flag_8719.fts_charger_mode_flag ? "On" : "Off");
...
static ssize_t fts_touch_charger_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        if (!g_fts_mode_flag_8719.fts_charger_mode_flag) {
            FTS_INFO("[Mode]enter charger mode");
            ret = fts_enter_charger_mode(client, true);
            if (ret >= 0) {
                g_fts_mode_flag_8719.fts_charger_mode_flag = true;
            }
        }
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        if (g_fts_mode_flag_8719.fts_charger_mode_flag) {
            ...
            if (ret >= 0) {
                g_fts_mode_flag_8719.fts_charger_mode_flag = false;
            }
        }
    }
    FTS_INFO("[Mode]charger mode status: %d", g_fts_mode_flag_8719.fts_charger_mode_flag);
    return count;
}
